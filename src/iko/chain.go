...
type ChainDB interface {
	...
	Head() (transaction.Wrapper, error)
	...
	// 'check' returns nil.
	AddTx(txWrapper transaction.Wrapper, check TxChecker) error

	// GetTxOfHash should obtain a transaction of a given hash.
	// It should return an error when the tx doesn't exist.
	GetTxOfHash(txID transaction.ID) (transaction.Wrapper, error)
	...
	GetTxOfSeq(seq uint64) (transaction.Wrapper, error)
	...
	TxChan() <-chan *transaction.Wrapper
	...
	GetTxsOfSeqRange(startSeq uint64, pageSize uint64) ([]transaction.Wrapper, error)
}
